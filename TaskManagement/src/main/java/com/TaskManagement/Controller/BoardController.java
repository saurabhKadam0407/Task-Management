...
public class BoardController {
	...
	public ResponseEntity<String>moveCard(@PathVariable Long id,
		...
		int postion= Integer.parseInt(String.valueOf(body.getOrDefault("position", "0")));
		boardService.moveCards(id, cardId, columnId, postion, user == null? "system":user);
		
		return ResponseEntity.ok("Moved");
	}
	
	@PostMapping("/{id}/columns/{colId}/records")
	public ResponseEntity<String>recordColumn(@PathVariable Long id, @PathVariable Long colId,@RequestBody List<Long> orderededCardId){
		boardService.recordCoumn(id, colId, orderededCardId);
		return ResponseEntity.ok("Recorded");
	}
	
	
	@PostMapping("/sprint/{sprintId}/start")
	public ResponseEntity<String>satrtSprint(@PathVariable Long sprintId){
		boardService.startSprint(sprintId);
		return ResponseEntity.ok("Sprint start");
	}
	
	@PostMapping("/sprint/{sprintId}/close")
	public ResponseEntity<String>closeSprint(@PathVariable Long sprintId){
		boardService.closeSprint(sprintId);
		return ResponseEntity.ok("Sprint close");
	}
